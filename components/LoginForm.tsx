          ...
          </div>
          {
            props.type === 'password' ? (
              <div className="flex justify-end" onClick={() => setIsOpen(true)}>
                <h1 className='mr-5 mt-2 text-[#7FB5FF] opacity-80 font-chakraPetch text-xs font-normal uppercase tracking-widest hover:cursor-pointer'>forgot password?</h1>
              </div>
            ) : null
          }
        </div>
  ...
  return (
    <div className='w-full mt-8 md:mt-0'>
      <div className='border-2 border-[#0F1421] rounded-[40px] p-12'>
        ...
        <div>
          <PasswordInput title="user name or email" type="email" />
          <PasswordInput title="password" type="password" />
        </div>
      </div>
      <div className='flex flex-row w-full justify-between mt-4 md:mt-12 space-x-6'>        
        <ButtonLayout text="login account" isCancel={false} active={false} onClick={() => console.log('clicked')} />
      </div>

      <ForgotPasswordModal isOpen={isOpen} setOpen={setIsOpen} />
